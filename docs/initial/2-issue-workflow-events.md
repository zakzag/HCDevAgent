...
| 10 | Planning                             | `PLAN`                           | field update                       | Agent            | `updateCustomField`  | AI writes machine-parseable plan to `Implementation Plan For AI` custom field |
| 11 | Planning                             | `PLAN`                           | → `PLAN REVIEW`                    | Agent            | `transitionStatus`   | Plan ready for human review                                             |
| 12 | Planning                             | `PLAN REVIEW`                    | → `READY FOR IMPLEMENTATION`       | Human            | —                    | Happy Path: Human approves the plan in Jira                             |
| 13 | Planning                             | `PLAN REVIEW`                    | new comment                        | Human            | —                    | Unhappy Path: Human writes comment why plan was rejected                |
| 14 | Planning                             | `PLAN REVIEW`                    | → `PLAN`                           | Human            | —                    | Unhappy Path: Human rejects the plan — agent re-runs Plan generation    |
| 15 | Implementation — start (or re-entry) | `READY FOR IMPLEMENTATION`       | → `IN PROGRESS`                    | Agent            | `transitionStatus`   | Agent starts Phase 3 — code implementation                              |
